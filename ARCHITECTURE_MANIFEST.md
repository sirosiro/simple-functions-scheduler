...
#### 4.5. Matrix State Machine ライブラリ
*   **詳細仕様:** `libs/matrix/ARCHITECTURE_MANIFEST.md` を参照してください。
    *   **概要:** 「モード」「状態」「イベント」を軸とする3次元マトリクス構造を用いた、決定論的な状態遷移管理機能を提供します。ログ出力の外部注入をサポートし、高いポータビリティと保守性を両立します。
